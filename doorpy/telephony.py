# doorpy/telephony.py
import pjsua as pj  # Import del binding PJSIP (pjsua)
import threading
import logging

class Telephony:
    def __init__(self, cfg, on_ring, on_dtmf):
        self.sip_cfg = cfg['sip']
        self.on_ring = on_ring
        self.on_dtmf = on_dtmf
        self.lib = pj.Lib()
        # Indici dispositivi audio definiti nel config (capture, playback)
        cap_dev, play_dev = cfg['audio_device_index']
        self.audio_dev = (cap_dev, play_dev)
